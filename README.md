...
**✨ 黑科技体验**：你完全不需要手动去开各家的网页，插件会在后台**自动为你唤醒**对应的 AI 标签页并开始静默工作。你只需喝口水，欣赏多路顶级 AI 同屏为你疯狂输出的震撼场面！

### 第 4 步：按需加通道、重试、总结

回答过程中如果想追加某个 AI，可以直接打开对应通道加入当前轮次。某一路失败时，也可以只重试这个通道。多个通道回答完成后，可以让「AI 裁判」归纳共识、分歧和最终建议。
...
在「通道列表」里，每个 AI 旁边都有独立的开关按钮，可以自由选择开启/关闭任意 AI，想对比几个就对比几个。通道列表支持全选、反选和分组展示，常用组合切换起来更快。
...
开启「导播模式」后，插件会自动把焦点切到正在输出的 AI 页面，尽量减少浏览器对后台标签页的限速。多通道同时跑长回答时，这个功能尤其有用。
...
网页模式依赖各家官网状态，偶尔会遇到登录过期、页面加载慢或发送失败。现在每个失败通道都可以单独重试，不需要把整轮对话推倒重来。
