...
import { Modal, Segmented, Input, Select, Button, Alert, Space } from 'antd';
...
export default function ChannelSettingsModal() {
  ...
  const modeOptions = [
    ...
    {
      label: !supportsApi ? 'API（暂不支持）' : !apiKey.trim() ? 'API（需先填 Key）' : 'API 模式',
      value: 'api',
    ...
    <Modal
      ...
      title={
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontSize: 16, fontWeight: 500 }}>{providerLabel} 设置</span>
        </div>
      }
    >
      <div style={{ paddingTop: 8 }}>
        ...
        <div style={{ marginBottom: 20 }}>
          <div style={{ fontSize: 13, fontWeight: 500, color: '#334155', marginBottom: 8 }}>接入模式</div>
          <Segmented
            block
            options={modeOptions}
            value={mode}
        ...
        {supportsApi && (
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: '#334155' }}>API 配置</div>
