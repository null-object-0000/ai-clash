  ...
  <div class="bg-white rounded-xl border border-slate-200/60 shadow-sm overflow-hidden transition-all duration-300">
      ...
      <div class="flex items-center gap-2.5">
        <span v-if="status === 'running'" class="relative flex h-3 w-3 ml-1">
          <span
            ...
            :class="`bg-${themeColor}-400`"></span>
          <span class="relative inline-flex rounded-full h-3 w-3" :class="`bg-${themeColor}-500`"></span>
        </span>
        ...
        <span class="text-[13px] font-medium" :class="`text-${themeColor}-600`">
          {{ providerName }} {{ status === 'running' ? '正在输出...' : '(已完成)' }}
        </span>
    ...
    <div v-show="isOpen"
      ...
      <div class="text-slate-600 leading-relaxed prose prose-sm max-w-none font-mono text-[12px] break-words"
        :class="`prose-${themeColor}`">
        <span v-html="renderMarkdown(response) || (status === 'running' ? (stage === 'responding' ? '' : stageLabel) : '等待连接网页端...')"></span>
        <span v-if="status === 'running' && stage === 'responding'"
          class="inline-block w-1.5 h-3.5 ml-0.5 animate-pulse align-middle"
          :class="`bg-${themeColor}-500`"></span>
      </div>
    </div>
  </div>
</template>
